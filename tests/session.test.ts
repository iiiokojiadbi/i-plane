...
import { resolveConfig, resolveSessionConfig, type SessionConfig } from "../src/config.ts";
...
test("credential modes are separate", () => {
  ...
  try {
    ...
    expect(resolveSessionConfig(input, directory).token.value).toBe("");
...
test("offline unknown-flag diagnostics mask cached session cookies and CSRF before authentication", async () => {
  transport();
  const client = new SessionClient(config());
  const session = await client.session();
  for (const secret of [session.sessionId, session.csrf]) {
