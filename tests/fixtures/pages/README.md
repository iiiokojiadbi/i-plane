...
`knowledge-review.json` was generated by the real Plane converter with the runtime
reader loaded. It covers quotes, markup characters, newlines and fence characters
inside the source. `tests/page-review.test.ts` checks the exact input HTML and the
lossless Markdown round trip. The mutation gate replaces each direction with a
paragraph and requires the focused tests to fail.
