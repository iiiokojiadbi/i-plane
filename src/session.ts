...
import type { SessionConfig, SessionSettings } from "./config.ts";
...
/** Protect cached credentials before even offline argument diagnostics run. */
export const guardCachedSessions = async (directory: string): Promise<void> => {
  const names = await readdir(directory).catch(() => []);
  for (const name of names)
    if (/^[a-f0-9]{64}\.json$/.test(name)) await loadSession(join(directory, name));
};

export const inspectSession = async (baseUrl: string, settings: SessionSettings) => ({
  login: { configured: settings.login !== undefined, origin: settings.login?.origin },
  password: { configured: settings.password !== undefined, origin: settings.password?.origin },
  cacheDirectory: settings.cacheDirectory,
  state:
    settings.login &&
    (await loadSession(cacheFile(baseUrl, settings.login.value, settings.cacheDirectory)))
      ? "cached, not verified"
      : "not cached",
});
...
export class SessionClient {
  ...
  async raw(path: string, init: RequestInit = {}): Promise<RawReply> {
    ...
    try {
      ...
      const deadline = new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
          controller.abort();
          reject(new PlaneError(`Session request timed out after ${DEADLINE / 1000}s: ${path}`));
        }, DEADLINE);
      });
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const reply = await this.raw(target, {
  ...
  async listAll<T>(path: string): Promise<ReadonlyArray<T>> {
    const rows: T[] = [];
    const seen = new Set<string>();
    let cursor: string | undefined;
    do {
      const page = await this.request<unknown>(path, { query: cursor ? { cursor } : undefined });
